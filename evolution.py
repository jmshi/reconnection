...
def findCurrentSheet(j2,fname):
  ...
  #tstart = time.time()
  #jth = estimateJth(j2)
  jth = 1.3879e4  # specify one for t=40case j^2
  jth = 5.8987e4  # for t=45 case j^2
  #tend = time.time()
  #print 'time cost for the estimate jth: ',tend-tstart,' seconds'

  # save the jth, <jsheet>, and <jtot> in table
  fhandler=open(fname[:-9]+'jth.tab','a')
  frame = int(fname[-9:-4])
...
def findNullPoint(bx,by,bz,nproc=0):
  """
...
if __name__=='__main__':
    ...
    for frame in np.arange(ts,te,tstride):
      fname = fdir+targ+'/'+basename+str(frame).zfill(5)+'.vtk'	    
      bx,by,bz,j2 = loadData(fname)
