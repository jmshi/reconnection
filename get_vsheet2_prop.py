...
def dimensions_jsheet(num):
  ...
  if (len(xlocs[:,0])<nlim):
    ...
    eps  = np.average(nu*np.array(js.values()))
...
if __name__=='__main__':
  if len( sys.argv ) < 4:
    print "calling sequence: python get_vsheet_prop.py targ,ts,te,tstride,[nu=2.5e-4],[nlim=8]" 
    exit(  )
  targ = sys.argv[1]
  ...
  for frame in np.arange(ts,te,tstride):
    fname = fdir+targ+'/'+basename+str(frame).zfill(5)+'.athdf.vlist2.p'	    
    ...
    pname = fdir+targ+'/'+'vprop2.'+str(frame).zfill(5)+'.p'
  ...
  pname = fdir+targ+'/'+'vprop2_ts='+str(ts)+'_te='+str(te)+'.p'
